        ...
        switch(comandoSemPrefixo){
            ...
            case "infobot":
                try{
                    ...
                    resposta += criarTexto(comandos_info.admin.infobot.msgs.resposta_inferior, usuariosBloqueados.length, infoBot.cmds_executados.toLocaleString("pt-BR"), numero_dono.replace("@s.whatsapp.net", ""))
            ...
            case "infofig":
                try{
                    let respostaDadosFig = criarTexto(
                        comandos_info.admin.infofig.msgs.resposta_titulo,
                        botInfo.contagem_fig.total.toLocaleString("pt-BR"),
                        botInfo.contagem_fig.mes.geral.toLocaleString("pt-BR"),
                        botInfo.contagem_fig.mes.s.toLocaleString("pt-BR"),
                        botInfo.contagem_fig.mes.ssf.toLocaleString("pt-BR"),
                        botInfo.contagem_fig.mes.sgif.toLocaleString("pt-BR"),
                        botInfo.contagem_fig.mes.simg.toLocaleString("pt-BR"),
                        botInfo.contagem_fig.mes.snome.toLocaleString("pt-BR"),
                        botInfo.contagem_fig.mes.emojimix.toLocaleString("pt-BR"),
                    )
                    if(!args.length) return await socket.enviarTexto(c, id_chat, respostaDadosFig)
                    let qtdUsuarios = texto_recebido
                    if(isNaN(qtdUsuarios)) return await socket.responderTexto(c, id_chat, comandos_info.admin.infofig.msgs.erro_qtd , mensagem)
                    if(qtdUsuarios < 1) return await socket.responderTexto(c, id_chat, comandos_info.admin.infofig.msgs.erro_minimo, mensagem)
                    let usuariosListados = await usuarios.obterMaioresFigurinhasMes(qtdUsuarios)
                    let qtdUsuariosListados = usuariosListados.length
                    if(qtdUsuariosListados > 0){
                        let numero_lista = 1
                        respostaDadosFig += criarTexto(comandos_info.admin.infofig.msgs.ranking_titulo, qtdUsuariosListados)
                        for(let usuario of usuariosListados){
                            let porcentagemUsuario = ((usuario.figs_feitas_mes * 100)/botInfo.contagem_fig.mes.geral).toFixed(2)
                            respostaDadosFig += criarTexto(comandos_info.admin.infofig.msgs.resposta_itens, numero_lista, usuario.nome, usuario.figs_feitas_mes.toLocaleString("pt-BR"), porcentagemUsuario)
                            numero_lista++
                        }
                        await socket.enviarTexto(c, id_chat, respostaDadosFig)
                    } else {
                        await socket.responderTexto(c, id_chat, comandos_info.admin.infofig.msgs.sem_usuarios, mensagem)
                    }
                } catch(err){
                    throw err
                }
