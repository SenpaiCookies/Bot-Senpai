        ...
        switch(comandoSemPrefixo){
            case 'snome':
                try{
                    ...
                    await bot.atualizarFigurinhasFeitas(comandoSemPrefixo)
                    await usuarios.adicionarFigurinhasFeitas(remetente)
                } catch(err){
            ...
            case 's':
                try{
                    ...
                    let {resultado: resultadoSticker} = await api.Stickers.criarSticker(bufferMidia, {pack: "🇯🇵 Twitter 𝕏", autor: "@BotDoSenpai", fps: 9, tipo: tipoFigurinha})
                    await socket.enviarFigurinha(c, id_chat, resultadoSticker)
                    await bot.atualizarFigurinhasFeitas(dadosMensagem.tipo == tiposMensagem.imagem ? "s" : "sgif")
                    await usuarios.adicionarFigurinhasFeitas(remetente)
                } catch(err){
                    if(!err.erro) throw err
                    await socket.responderTexto(c, id_chat, criarTexto(comandos_info.outros.erro_api, comando, err.erro) , mensagem)
            ...
            case 'emojimix':
                try{
                    ...
                    let {resultado : resultadoSticker} = await api.Stickers.criarSticker(resultadoEmoji, {pack: "🇯🇵 Twitter 𝕏", autor: "@BotDoSenpai"})
                    await socket.enviarFigurinha(c, id_chat, resultadoSticker)
                    await bot.atualizarFigurinhasFeitas(comandoSemPrefixo)
                    await usuarios.adicionarFigurinhasFeitas(remetente)
                } catch(err){
            ...
            case "ssf":
                try{
                    ...
                    let {resultado : resultadoSticker} = await api.Stickers.criarSticker(resultadoImg, {pack: "🇯🇵 Twitter 𝕏", autor: "@BotDoSenpai"})
                    await socket.enviarFigurinha(c, id_chat, resultadoSticker)
                    await bot.atualizarFigurinhasFeitas(comandoSemPrefixo)
                    await usuarios.adicionarFigurinhasFeitas(remetente)
                } catch(err){
...
export const autoSticker = async(c, mensagemBaileys, botInfo)=>{
    ...
    const {id_chat, tipo, mensagem, midia, remetente} = mensagemBaileys
    ...
    try{
        ...
        if(tipo == tiposMensagem.imagem || tipo == tiposMensagem.video){
            ...
            let {resultado: resultadoSticker} = await api.Stickers.criarSticker(bufferMidia, {pack: "🇯🇵 Twitter 𝕏", autor: "@BotDoSenpai", fps: 9})
            await socket.enviarFigurinha(c, id_chat, resultadoSticker)
            await bot.atualizarFigurinhasFeitas(tipo == tiposMensagem.imagem ? "s" : "sgif")
            await usuarios.adicionarFigurinhasFeitas(remetente)
        }
    } catch(err){
        if(!err.erro) throw err
