...
import {criarTexto, erroComandoMsg, timestampParaData, consoleErro, versaoAtual, obterTimestampMesSeguinte} from '../lib/util.js'
...
export const info = async(c, mensagemBaileys, botInfo) => {
    ...
    try{
        switch(comandoSemPrefixo){
            case `info`:
                try{
                    console.log(obterTimestampMesSeguinte())
                    let infoBot = botInfo
                    let botInicializacaoData = timestampParaData(infoBot.iniciado)
                    let resposta = criarTexto(comandos_info.info.info.msgs.resposta, nome_adm?.trim(), nome_bot?.trim(), botInicializacaoData)
            ...
            case `menu`:
                try{
                    ...
                    if(!args.length){
                        ...
                        await socket.enviarArquivoLocal(c, tiposMensagem.imagem, id_chat, "./menu.png", dadosResposta+menuResposta)
                    } else {
                        ...
                        switch(usuarioOpcao){
                            ...
                            case "4":
                                if(mensagem_grupo) {
                                    menuResposta = menu.menuGrupo(usuario_admin, botInfo)
                                    await socket.enviarTexto(c, id_chat, dadosResposta+menuResposta)
                                } else {
                                ...
                                break
                            case "5":
                                socket.enviarLinkComPrevia(c, id_chat, "https://whatsapp.com/channel/0029VatyGWjFcow5imozTp2r")
                                break
                        }
                    }
                } catch(err){
