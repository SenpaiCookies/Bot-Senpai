...
export class Usuario {
    ...
    async addFigurinhasFeitas(id_usuario){
        await db.updateAsync({id_usuario}, {$inc: {figs_feitas_total: 1, figs_feitas_mes: 1}})
    }
    ...
    async resetarFigurinhasFeitasMes(){
        await db.updateAsync({}, {$set:{figs_feitas_mes : 0}}, {multi: true})
    }
    ...
    async obterMaioresFigurinhasMes(){
        return await db.findAsync({}).sort({figs_feitas_mes: -1})
    }
