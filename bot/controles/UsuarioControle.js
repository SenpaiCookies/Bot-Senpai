...
export class UsuarioControle{
    ...
    async adicionarFigurinhasFeitas(id_usuario){
        await this.usuario.addFigurinhasFeitas(id_usuario)
    }
    ...
    async resetarFigurinhasFeitasMes(){
        await this.usuario.resetarFigurinhasFeitasMes()
    }

    async obterMaioresFigurinhasMes(qtd) {
        let usuarios = await this.usuario.obterMaioresFigurinhasMes()
        return usuarios.length >= qtd ? usuarios.slice(0, qtd) : usuarios
    }
