...
import {consoleErro, criarTexto, corTexto, obterTimestampMesSeguinte} from '../lib/util.js'
...
export class BotControle{
        ...
        const bot = {
            ...
            contagem_fig:{
                ...
                mes : {
                    ...
                    simg: 0,
                    emojimix: 0,
                    expiracao_mes: obterTimestampMesSeguinte()
                },
            },
            limite_diario:{
                status: false,
    ...
    async atualizarFigurinhasFeitas(comando){
        await this.bot.atualizarFigurinhasFeitas(comando)
    }
    ...
    async verificarFigurinhasMensal(botInfo){
        let agora = new Date().getTime()
        let expiracao_mes = botInfo.contagem_fig.mes.expiracao_mes
        if(agora > expiracao_mes){
            await new UsuarioControle().resetarFigurinhasFeitasMes()
            await this.bot.resetarFigurinhasFeitasMes()
            await this.bot.alterarProximoMesFigurinha(obterTimestampMesSeguinte())
        }
    }
